"""SENTINEL v3 — Drought Module (Enhanced)
ENHANCED: Live-reactive severity · SPI time series · Water stress model · Crop impact · Groundwater depletion
"""
...
from utils.charts import make_gauge, make_radar, make_drought_timeline, make_globe, make_donut, _hex_to_rgb
...
def make_spi_timeseries(spi, color):
    """Simulated SPI time series with drought classification bands"""
    ...
    for y_min, y_max, label, band_color in [(-4,-2,"Extreme Drought","rgba(255,34,68,0.15)"),
                                              ...
                                              (1,4,"Wet Period","rgba(0,255,136,0.08)")]:
        fig.add_hrect(y0=y_min,y1=y_max,fillcolor=band_color,layer="below",line_width=0,annotation_text=label,
            annotation_font={"family":"Share Tech Mono","size":8,"color":"rgba(255,255,255,0.4)"},annotation_position="right")
    ...
    fig.add_hline(y=spi,line_color="white",line_dash="dash",line_width=1.5,
        annotation_text=f" Current SPI: {spi:.1f}",
        annotation_font={"family":"Share Tech Mono","size":10,"color":"white"})
    fig.update_layout(title=dict(text="📉 Standardized Precipitation Index — 5-Year Record",font={"family":"Orbitron","size":13,"color":"#8ab4d0"}),
        xaxis=dict(title="Months",gridcolor="#0d2a44"),yaxis=dict(title="SPI",gridcolor="#0d2a44",range=[-4,4]),
...
def make_water_stress_chart(spi, evapotrans, precip_deficit, ndvi, color):
    """Water stress indicators stacked chart"""
    categories=["Precipitation\nDeficit","Evapotrans-\npiration","Vegetation\nStress","Soil\nMoisture Loss","Economic\nImpact"]
    # Derived values
    precip_stress=precip_deficit
    et_stress=evapotrans/10*100
    veg_stress=max(0,(0.6-ndvi)/0.6*100) if ndvi<0.6 else 0
    soil_stress=max(0,min(100,precip_deficit*0.8+(-spi)*20))
    econ_stress=max(0,precip_deficit*0.5+veg_stress*0.3+(-spi)*15)
    values=[precip_stress,et_stress,veg_stress,soil_stress,econ_stress]
    bar_colors=[f"rgba({_hex_to_rgb(color)},{0.4+v/100*0.6:.2f})" for v in values]
    fig=go.Figure(go.Bar(x=categories,y=values,
        marker=dict(color=bar_colors,cornerradius=6,line=dict(color=color,width=0.5)),
        text=[f"{v:.0f}%" for v in values],textposition="auto",
        textfont=dict(family="Share Tech Mono",size=11)))
    fig.add_hline(y=50,line_dash="dash",line_color="#ffd700",line_width=1,
        annotation_text=" Stress Threshold",annotation_font={"family":"Share Tech Mono","size":9,"color":"#ffd700"})
    fig.update_layout(title=dict(text="💧 Multi-Sector Water Stress Analysis",font={"family":"Orbitron","size":13,"color":"#8ab4d0"}),
...
def make_groundwater_chart(spi, precip_deficit, temp_anomaly, color):
    ...
    # Depletion model
    annual_recharge=max(0,(spi+3)/6)*800  # mm/year
    annual_demand=800+temp_anomaly*50+precip_deficit*6
    net_change=(annual_recharge-annual_demand)/1000  # m/year
    level=np.cumsum(np.ones(20)*net_change)
    fig=go.Figure()
    fig.add_trace(go.Scatter(x=years,y=level,mode="lines",name="Water Table (m change)",
        line=dict(color=color if net_change<0 else "#00ff88",width=2.5),
        fill="tozeroy",fillcolor=f"rgba({_hex_to_rgb(color if net_change<0 else '#00ff88')},0.1)"))
    fig.add_hline(y=0,line_color="rgba(255,255,255,0.4)",line_width=1,line_dash="dot")
    fig.update_layout(title=dict(text=f"💧 Groundwater Level Projection ({'+' if net_change>=0 else ''}{net_change:.2f}m/yr)",
        font={"family":"Orbitron","size":13,"color":"#8ab4d0"}),
        xaxis=dict(title="Years from Now",gridcolor="#0d2a44"),
        yaxis=dict(title="Water Table Change (m)",gridcolor="#0d2a44"),
        paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
...
def make_crop_impact_chart(severity, color):
    """Crop yield reduction by type"""
    crops=["Wheat","Maize","Rice","Soybeans","Cotton","Vegetables"]
    sensitivity=[0.8,0.9,0.85,0.75,0.7,1.0]  # drought sensitivity
    sev_map={"No Drought":0,"Mild":0.1,"Moderate":0.3,"Severe":0.6,"Extreme":0.85}
    sev_frac=sev_map.get(severity,0)
    yield_loss=[round(s*sev_frac*100,1) for s in sensitivity]
    bar_colors=["#00ff88" if y<20 else "#ffd700" if y<40 else "#ff7700" if y<60 else "#ff2244" for y in yield_loss]
    fig=go.Figure(go.Bar(x=crops,y=yield_loss,marker=dict(color=bar_colors,cornerradius=6),
        text=[f"-{y}%" for y in yield_loss],textposition="auto",textfont=dict(family="Share Tech Mono",size=10)))
    fig.update_layout(title=dict(text=f"🌾 Estimated Crop Yield Loss — {severity}",font={"family":"Orbitron","size":13,"color":"#8ab4d0"}),
        paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",font=dict(family="Share Tech Mono",color="#8ab4d0"),
        xaxis=dict(gridcolor="#0d2a44"),yaxis=dict(title="Yield Reduction (%)",gridcolor="#0d2a44",range=[0,100]),
...
def render_drought_page(db: DBManager):
    st.markdown(f"""<div class="dis-header" style="--dh-color:{COLOR};--dh-bg:{THEME['bg']};">
      ...
      <div class="dis-subtitle">SPI analysis · Water stress · Groundwater depletion · Crop impact · Live reactive</div></div>
      <div class="dis-badges"><div class="dis-badge">SPI INDEX</div>
      <div class="dis-badge">PALMER DROUGHT</div><div class="dis-badge">LIVE REACTIVE</div></div>
    </div>""", unsafe_allow_html=True)

    model = load_model("drought")
    tab1, tab2, tab3, tab4 = st.tabs(["🔮 Predict","🌍 Drought Map","💧 Water Analysis","📊 History"])
    ...
    with tab1:
        ...
        with col2:
            ...
            if st.button("🌐 Fetch Live Weather",key="dr_live2"):
                ...
                if data and "current" in data:
                    c=data["current"]
                    temp_live=c.get("temperature_2m",30); temp_anomaly=round(temp_live-25,1)
                    st.success(f"Live: {temp_live}°C (anomaly: {temp_anomaly:+.1f}°C)")
        ...
        if st.button("🏜️ Save & Full Drought Assessment",type="primary",key="dr_run2",use_container_width=True):
            ...
            db.save_drought({"region":region,"spi":spi,"temp_anomaly":temp_anomaly,
                             "precip_deficit":precip_deficit,"evapotranspiration":evapotrans,
                             "ndvi":ndvi,"severity_score":score,"severity_level":sev})
            ...
            st.markdown(f"""<div class="pred-card" style="--pc-color:{col_r};">
              ...
              <span class="pred-unit">Palmer Drought Severity Index analog</span>
              ...
              <span class="pred-tag">NDVI: {ndvi:.2f}</span></div></div>""",unsafe_allow_html=True)
            c1,c2=st.columns(2)
    ...
    with tab2:
        ...
        drought_zones=[
            {"Region":"Sahel, Africa","lat":14.0,"lon":10.0,"severity":2.1},
        ...
        df_dz["color"]=[SEV_COLORS.get(s,"#aaa") for s in df_dz["sev_label"]]
        trace=go.Scattergeo(lat=df_dz["lat"],lon=df_dz["lon"],mode="markers+text",
            marker=dict(size=df_dz["severity"]*15+8,color=df_dz["severity"],
                        colorscale="OrRd",cmin=0,cmax=2.5,
        ...
        fig_dz=go.Figure(go.Bar(x=df_dz["Region"],y=df_dz["severity"],
            marker=dict(color=df_dz["severity"],colorscale="OrRd",cmin=0,cmax=2.5,cornerradius=5),
            text=df_dz["sev_label"],textposition="auto",textfont=dict(family="Share Tech Mono",size=9)))
        fig_dz.update_layout(title=dict(text="Drought Severity by Region",font={"family":"Orbitron","size":12,"color":"#8ab4d0"}),
            paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",font=dict(family="Share Tech Mono",color="#8ab4d0"),
            xaxis=dict(gridcolor="#0d2a44",tickangle=-30),yaxis=dict(title="Severity Score",gridcolor="#0d2a44"),
    ...
    with tab3:
        ...
        wa_score=max(0,(-wa_spi-1)*0.5+wa_def/200+wa_et/20); wa_sev=score_to_sev(wa_score); wa_color=SEV_COLORS.get(wa_sev,COLOR)
        c1,c2=st.columns(2)
    ...
    with tab4:
        ...
        if history:
            ...
            c1,c2=st.columns(2)
            with c1: st.plotly_chart(make_timeseries(df_h.head(25),"timestamp","severity_score",COLOR,"Drought Severity History"),use_container_width=True)
            with c2:
                rc=df_h["severity_level"].value_counts()
            ...
            st.markdown('<div class="info-box">No drought prediction history yet.</div>',unsafe_allow_html=True)
